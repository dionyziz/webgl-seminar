define( [ 'libs/math' ], function( math ) {
	...
	exports.parallepipedInterfereArea = function( box1, box2, pos1, pos2 ) {
		var i, p1 = [], p2 = [], 
        ind = [ 0,1,2,
                1,2,3,
                0,1,4,
	...
	areTriesColliding = exports.areTriesColliding = function( tri1, tri2 ) {
		...
		for( var i = 0; i < 6; i++  ) {
			for ( var u = i+1; u < 6; u++ ) {
				for ( var j = u+1; j < 6; j++ ) {
					if ( isSeperatingPlane( [ allPoints[ i ], allPoints[ u ], allPoints[ j ] ], tri1, tri2 ) ) {
						return false;
					}	
				}
			}
