define( [ 'libs/extender', 'libs/math', 'physics', 'models/transformable' ], function( inherits, math, physics, Transformable ) {
    ...
    var Mesh = function( vertices, indices ) {
        ...
        this.scale = function( x, y, z ) {
            ...
            var scaleMatrix = tetra.scaleMatrix();
            for ( var i = 0, l = this.vertices.length; i < l; i += 3 ) {
                res = [];
                mat4.multiplyVec3( scaleMatrix,
                                   [ this.vertices[ i ], this.vertices[ i + 1 ], this.vertices[ i + 2 ] ],
                                   res );
                this.vertices[ i ] = res[ 0 ];
                this.vertices[ i + 1 ] = res[ 1 ];
        ...
        this.setHullFromMesh = function() {
            ...
            sphereCenter = [ 0, 0, 0 ];
            sphereRadius = physics.bounds.findBoundingSphere( this.vertices );//Math.max( dian3.metro( rect[ 1 ] ), dian3.metro( rect[ 0 ] ) );//0.5*dian3.metro( dian3.subtract( rect[ 1 ], rect[ 0 ] ) );//<-- needs unit testing
            validBox = true;
        };
